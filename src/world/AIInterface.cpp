...
void AIInterface::Update(uint32 p_time)
{
    ...
    if (m_AIType == AITYPE_TOTEM)
    {
        ...
        if (p_time >= m_totemspelltimer)
        {
            ...
            {
                //we set no target and see if we managed to find a new one
                SetNextTarget((Unit*)NULL);
    ...
    if (m_AIState == STATE_EVADE)
    {
        ...
        if (tdist <= 4.0f/*2.0*/)
        {
            ...
            */
            // Set health to full if they at their last location before attacking
    ...
    if (cansee && GetNextTarget() && GetNextTarget()->isAlive() && m_AIState != STATE_EVADE && !m_Unit->isCasting())
    {
        ...
        switch (agent)
        {
            case AGENT_MELEE:
            {
                ...
                float combatReach = _CalcCombatRange(GetNextTarget(), false);

                if (distance <= combatReach + DISTANCE_TOO_SMAL_TO_WALK)
                { // Target is in range -> Attack
                    if (UnitToFollow != NULL)
                    {
                        UnitToFollow = NULL; // remove follow target
                        m_lastFollowX = m_lastFollowY = 0;
                    }
                    ...
                    if (m_Unit->isAttackReady(false) && !m_fleeTimer)
                    {
                        ...
                        if (infront)
                        {
                            ...
                            // take initial health to determine if the strike hit
                            ...
                            if (GetNextTarget() && m_Unit->m_factionDBC
                                && !(m_Unit->m_factionDBC->RepListId == -1 && m_Unit->m_faction->FriendlyMask == 0
                                && m_Unit->m_faction->HostileMask == 0) /* neutral creature */
                                && GetNextTarget()->IsPlayer() && !m_Unit->IsPet()
                                && health_before_strike > GetNextTarget()->GetUInt32Value(UNIT_FIELD_HEALTH)
                ...
                else // Target is out of range -> Run to it
                {
                    m_moveRun = true;
                    Unit* target = GetNextTarget();
                    float xDiff = target->GetPositionX() - m_Unit->GetPositionX();
                    float yDiff = target->GetPositionY() - m_Unit->GetPositionY();
                    float angle = 0.0f;
                    if (xDiff != 0)
                    {
                    ...
                    float xPos = target->GetPositionX() - target->GetModelHalfSize() * cos(angle);
                    float yPos = target->GetPositionY() - target->GetModelHalfSize() * sin(angle);
                    float zPos = target->GetPositionZ();
                    MoveTo(xPos, yPos, zPos, 0.0f);
                }
            }break;
            case AGENT_RANGED:
            {
            ...
            case AGENT_SPELL:
            {
                ...
                else // Target out of Range -> Run to it
                {
                    ...
                    if (distance > m_nextSpell->maxrange)
                        close_to_enemy = m_nextSpell->maxrange - DISTANCE_TOO_SMAL_TO_WALK;
                    else if (distance < m_nextSpell->minrange)
                        close_to_enemy = m_nextSpell->minrange + DISTANCE_TOO_SMAL_TO_WALK;
    ...
    if (sWorld.Collision && pUnit->IsPlayer())
    {
        ...
        if (m_Unit->GetMapMgr() != NULL)
        {
            if (!m_moveFly)
            {
                ...
                {
                    if (pUnit->GetTypeId() != TYPEID_PLAYER && target_land_z > m_Unit->GetMapMgr()->GetLiquidHeight(pUnit->GetPositionX(), pUnit->GetPositionY()))
                        return;
...
void AIInterface::_CalcDestinationAndMove(Unit *target, float dist)
{
    ...
    if (target && (target->GetTypeId() == TYPEID_UNIT || target->GetTypeId() == TYPEID_PLAYER))
    {
        ...
        if (abs(m_last_target_x - ResX) < DISTANCE_TOO_SMAL_TO_WALK
            && abs(m_last_target_y - ResY) < DISTANCE_TOO_SMAL_TO_WALK && m_creatureState == MOVING)
            return;
...
void AIInterface::UpdateMove()
{
    ...
    if (distance < DISTANCE_TOO_SMAL_TO_WALK)
...
void AIInterface::_UpdateMovement(uint32 p_time)
{
    ...
    if (m_creatureState == MOVING)
    {
        if (!m_moveTimer)
        {
            ...
            {
                // Dword: we send the update packet early if we are chasing a target (in combat) which causes "choppy" movement
                if (GetNextTarget() == 0)
                    UpdateMove();
            }
        }
